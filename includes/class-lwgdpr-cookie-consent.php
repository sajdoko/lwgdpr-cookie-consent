...
class Lw_Gdpr_Cookie_Consent {
	 ...
	 * @since    1.0
	 * @access   protected
	 * @var      Lw_Gdpr_Cookie_Consent_Loader    $loader    Maintains and registers all hooks for the plugin.
	 */
	protected $loader;
	 ...
	 *
	 * - Lw_Gdpr_Cookie_Consent_Loader. Orchestrates the hooks of the plugin.
	 * - Lw_Gdpr_Cookie_Consent_I18n. Defines internationalization functionality.
	 * - Lw_Gdpr_Cookie_Consent_Admin. Defines all hooks for the admin area.
	 * - Lw_Gdpr_Cookie_Consent_Public. Defines all hooks for the public side of the site.
	 *
	...
	private function load_dependencies() {
		 ...
		 * core plugin.
		 */
		require_once LW_GDPR_COOKIE_CONSENT_PLUGIN_PATH . 'includes/class-lwgdpr-cookie-consent-loader.php';

		/**
		 * The class responsible for defining internationalization functionality
		 * of the plugin.
		 */
		require_once LW_GDPR_COOKIE_CONSENT_PLUGIN_PATH . 'includes/class-lwgdpr-cookie-consent-i18n.php';

		/**
		 * The class responsible for defining all actions that occur in the admin area.
		 */
		require_once LW_GDPR_COOKIE_CONSENT_PLUGIN_PATH . 'admin/class-lwgdpr-cookie-consent-admin.php';

		/**
		 * The class responsible for defining all actions that occur in the public-facing
		 * side of the site.
		 */
		require_once LW_GDPR_COOKIE_CONSENT_PLUGIN_PATH . 'public/class-lwgdpr-cookie-consent-public.php';

		$this->loader = new Lw_Gdpr_Cookie_Consent_Loader();
	 ...
	 *
	 * Uses the Lw_Gdpr_Cookie_Consent_I18n class in order to set the domain and to register the hook
	 * with WordPress.
	 *
	 * @since    1.0
	 * @access   private
	...
	private function set_locale() {
		...
		$plugin_i18n = new Lw_Gdpr_Cookie_Consent_I18n();
	...
	private function define_admin_hooks() {
		...
		$plugin_admin = new Lw_Gdpr_Cookie_Consent_Admin( $this->get_plugin_name(), $this->get_version() );
		/**
		 * Load admin modules.
		 */
		$plugin_admin->admin_modules();
		$this->loader->add_action( 'init', $plugin_admin, 'lwgdpr_register_block_type' );
	...
	private function define_public_hooks() {
		...
		$plugin_public = new Lw_Gdpr_Cookie_Consent_Public( $this->get_plugin_name(), $this->get_version() );
		/**
		 * Load public modules.
		 */
		...
		if ( self::is_request( 'frontend' ) ) {
			...
			$this->loader->add_action( 'wp_footer', $plugin_public, 'gdprcookieconsent_inject_lwgdpr_script' );
		}
	}
	 ...
	 *
	 * @since     1.0
	 * @return    Lw_Gdpr_Cookie_Consent_Loader    Orchestrates the hooks of the plugin.
	 */
	 ...
	 */
	public static function lwgdpr_generate_settings_tabhead( $title_arr ) {
		$out_arr = array();
		foreach ( $title_arr as $k => $v ) {
			if ( 'lwgdpr-cookie-consent-buttons' === $k ) {
				$out_arr[ $k ] = $v;
				// tab head for modules.
				$out_arr = apply_filters( 'lwgdpr_module_settings_tabhead', $out_arr );
			} else {
				$out_arr[ $k ] = $v;
			}
		}
	 ...
	 */
	public static function lwgdpr_envelope_settings_tabcontent( $class, $target_id, $view_file = '', $html = '', $variables = array(), $need_submit_btn = 0, $error_message = '' ) {
		...
		$the_options = self::lwgdpr_get_settings();
		if ( 'script-blocker-advanced' === $target_id ) {
			?>
			<div class="<?php echo esc_attr( $class ); ?>" data-id="<?php echo esc_attr( $target_id ); ?>" lwgdpr_tab_frm_tgl-id="lwgdpr_usage_option" lwgdpr_tab_frm_tgl-val="lwgdpr">
		<?php } else { ?>
		...
		<?php } ?>
			...
			<?php
			if ( 1 === $need_submit_btn ) {
				include plugin_dir_path( LW_GDPR_COOKIE_CONSENT_PLUGIN_FILENAME ) . 'admin/views/admin-display-save-button.php';
			}
			?>
		</div>
		<?php
	 ...
	 * @since 1.0
	 */
	public static function lwgdpr_allowed_html() {
		$allowed_html = array(
	 ...
	 * @return array
	 */
	public static function lwgdpr_allowed_protocols() {
	 ...
	 * @return array|mixed
	 */
	public static function lwgdpr_get_default_settings( $key = '' ) {
		$settings = array(
			...
			'cookie_expiry'                   => '365',
			'show_again_div_id'               => '#lwgdpr-cookie-consent-show-again',
			'notify_animate_hide'             => true,
			'notify_animate_show'             => false,
			'notify_message'                  => addslashes( 'Utilizziamo i cookie per personalizzare contenuti ed annunci, per fornire funzionalità dei social media e per analizzare il nostro traffico. Condividiamo inoltre informazioni sul modo in cui utilizza il nostro sito con i nostri partner che si occupano di analisi dei dati web, pubblicità e social media, i quali potrebbero combinarle con altre informazioni che ha fornito loro o che hanno raccolto dal suo utilizzo dei loro servizi.' ),
			'notify_message_eprivacy'         => addslashes( 'Utilizziamo i cookie per personalizzare contenuti ed annunci, per fornire funzionalità dei social media e per analizzare il nostro traffico. Condividiamo inoltre informazioni sul modo in cui utilizza il nostro sito con i nostri partner che si occupano di analisi dei dati web, pubblicità e social media, i quali potrebbero combinarle con altre informazioni che ha fornito loro o che hanno raccolto dal suo utilizzo dei loro servizi.' ),
			'notify_message_ccpa'             => addslashes( 'In case of sale of your personal information, you may opt out by using the link' ),
			'notify_div_id'                   => '#lwgdpr-cookie-consent-bar',
...
Specifica l’ID del tuo consenso e la data di quando ci hai contattati per quanto riguarda il tuo consenso.' ) ),
	 ...
	 */
	public static function lwgdpr_sanitise_settings( $key, $value ) {
		$ret = null;
		switch ( $key ) {
			...
			case 'bar_heading_text':
				$ret = wp_kses( $value, self::lwgdpr_allowed_html(), self::lwgdpr_allowed_protocols() );
				break;
			// URLs only.
	 ...
	 * @return array|mixed
	 */
	public static function lwgdpr_get_settings() {
		$settings             = self::lwgdpr_get_default_settings();
		self::$stored_options = get_option( LW_GDPR_COOKIE_CONSENT_SETTINGS_FIELD );
		if ( ! empty( self::$stored_options ) ) {
			foreach ( self::$stored_options as $key => $option ) {
				$settings[ $key ] = self::lwgdpr_sanitise_settings( $key, $option );
			}
		}
		update_option( LW_GDPR_COOKIE_CONSENT_SETTINGS_FIELD, $settings );
		return $settings;
	}
	 ...
	 */
	public static function lwgdpr_su_hex_shift( $supplied_hex, $shift_method, $percentage = 50 ) {
	 ...
	 */
	public static function lwgdpr_get_json_settings() {
		$settings = self::lwgdpr_get_settings();
		...
		$slim_settings  = array(
			...
			'button_cancel_button_hover'      => ( self::lwgdpr_su_hex_shift( $settings['button_cancel_button_color'], 'down', 20 ) ),
			'button_confirm_button_color'     => $settings['button_confirm_button_color'],
			'button_confirm_button_hover'     => ( self::lwgdpr_su_hex_shift( $settings['button_confirm_button_color'], 'down', 20 ) ),
			'button_accept_link_color'        => $settings['button_accept_link_color'],
			'button_accept_button_color'      => $settings['button_accept_button_color'],
			'button_accept_button_hover'      => ( self::lwgdpr_su_hex_shift( $settings['button_accept_button_color'], 'down', 20 ) ),
			...
			'button_readmore_button_hover'    => ( self::lwgdpr_su_hex_shift( $settings['button_readmore_button_color'], 'down', 20 ) ),
			...
			'button_decline_button_hover'     => ( self::lwgdpr_su_hex_shift( $settings['button_decline_button_color'], 'down', 20 ) ),
			...
			'button_settings_button_hover'    => ( self::lwgdpr_su_hex_shift( $settings['button_settings_button_color'], 'down', 20 ) ),
			...
			'border_color'                    => ( self::lwgdpr_su_hex_shift( $settings['text'], 'up', 40 ) ),
			'background_color'                => ( self::lwgdpr_su_hex_shift( $settings['background'], 'down', 10 ) ),
